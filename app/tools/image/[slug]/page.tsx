...
import { isImageIndexed } from "@/lib/toolIndexing";

export const dynamicParams = false;
export const generateStaticParams = () => imageTools.map((tool) => ({ slug: tool.slug }));
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> { const tool = getImageTool((await params).slug); return tool ? { title: `${tool.name} - Free Online Tool`, description: tool.description, robots: isImageIndexed(tool) ? undefined : { index: false, follow: true }, alternates: { canonical: `/tools/image/${tool.slug}` } } : {}; }
