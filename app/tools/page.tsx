...
export const metadata: Metadata = { title: "Free Online Tools", description: "Browse 243 production-ready PDF, Image, Write, Video, and File tools.", alternates: { canonical: "/tools" } };

export default async function ToolsPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  const query = (await searchParams).q?.trim() || "";
  const keyword = query.toLowerCase();
  const matches = <T extends { name: string; description: string }>(tools: T[]) => keyword ? tools.filter((tool) => `${tool.name} ${tool.description}`.toLowerCase().includes(keyword)) : tools;
  const pdf = matches(pdfTools); const images = matches(imageTools); const writing = matches(writeTools); const videos = matches(videoTools); const files = matches(fileTools);
  const cards = [
