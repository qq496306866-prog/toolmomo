  ...
  return <header className="sticky top-0 z-40 border-b border-[#edf1f6] bg-white/95 backdrop-blur">
    <div className="mx-auto flex h-[72px] max-w-[1220px] items-center gap-4 px-5">
      ...
      <nav aria-label="Tool categories" className="ml-auto hidden h-full items-center md:flex">
        {siteCategories.map((category) => <div className="group relative flex h-full items-center" key={category.slug}>
          ...
          <div className="pointer-events-none absolute left-1/2 top-[64px] w-[620px] -translate-x-1/2 translate-y-2 rounded-[16px] bg-white p-5 opacity-0 shadow-[0_24px_70px_rgba(32,43,60,0.16)] ring-1 ring-[#e7edf5] transition group-focus-within:pointer-events-auto group-focus-within:translate-y-0 group-focus-within:opacity-100 group-hover:pointer-events-auto group-hover:translate-y-0 group-hover:opacity-100">
            ...
            <div className="mt-3 grid grid-cols-2 gap-2">{toolSets[category.slug].slice(0, 8).map((tool) => <a className="rounded-[10px] px-3 py-2 hover:bg-[#f7f9fc] focus:bg-[#f7f9fc]" href={category.slug === "pdf" ? `/tools/${tool.slug}` : `/tools/${category.slug}/${tool.slug}`} key={tool.slug}><span className="text-sm font-black text-[#263244]">{tool.name}</span><span className="mt-1 block text-xs font-semibold text-[#728197]">{tool.description}</span></a>)}</div>
      ...
      <details className="group/menu relative md:hidden"><summary aria-label="Open navigation" className="grid h-10 w-10 cursor-pointer list-none place-items-center rounded-full bg-[#f5f7fa] text-[#58667a] focus:outline-none focus:ring-2 focus:ring-[#ff5b34]"><span className="text-xl leading-none group-open/menu:hidden">☰</span><span className="hidden text-xl leading-none group-open/menu:block">×</span></summary><div className="absolute right-0 top-12 w-64 rounded-[14px] bg-white p-2 shadow-xl ring-1 ring-[#e7edf5]">{siteCategories.map((category) => <a className="flex min-h-11 items-center justify-between rounded-[10px] px-4 py-3 text-sm font-black hover:bg-[#f7f9fc] focus:bg-[#f7f9fc] focus:outline-none" href={category.href} key={category.slug}><span>{category.name}</span><span className="text-xs text-[#8a98aa]">{category.availableCount}</span></a>)}</div></details>
    </div>
  </header>;
}
